...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        # inputs shape is torch.Size([2, 3, 4])
        
        """
        index shape is torch.Size([1, 3, 2])
        divisor shape is tensor([  1., 100.])
        sin_embedding shape is torch.Size([1, 3, 2])
        cos_embedding shape is torch.Size([1, 3, 2])
        """
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, inputs, self_attention_mask=None):
        ...
        # attn should be same dim so  [batch_size, sequence_length, channels]
