...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        # print(f"queries's shape is {queries.shape}")
        # print(f"keys's shape is {keys.shape}")
        # print(f"keys's shape after t is {keys.transpose(-2, -1).shape}")
        
        similarity = th.matmul(queries, keys.transpose(-2, -1)) # Compute the similarity according to the QKV formula
        # print(f"similarity's shape is {similarity.shape}")
        ...
        weights = masked_similarity / th.sqrt(key_dim) # Turn the similarity into a normalized output. Remember that the last dim contains the features
